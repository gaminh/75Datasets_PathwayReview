...
# library(vioplot)
library(vioplot)
path = "/Users/minhnguyen/pathway_analysis_bias_survey/PathwayReview/"
setwd(path)

methods = c("FE", "KS", "WRS", "GSEA", "GSA", "PADOG", "SPIA", "ROntoTools", "CePaGSA", "CePaORA", "PathNet") #change order
Type <- c("Non-TB", "Non-TB", "Non-TB", "Non-TB", "Non-TB", "Non-TB", "Non-TB", "Non-TB", 
          "TB", "TB", "TB", "TB", "TB")

filename="dataset/datasetslist.txt"
dataSets75 = as.character(t(read.table(filename,header=F,sep="\t",stringsAsFactors=F)))

result <- list()
for (i in 1:length(methods)) {
  load(paste0(methods[i], "TargetPW_75Datasets.RData"))
  temp = data.frame(Rank = rank, pValue = pvalue)
  rownames(temp) = dataSets75
  result[[i]] = temp
  rm(rankWithoutNull)
  rm(pvalueWithoutNull)
}
names(result) = methods
# save(result, file = "ResultRankPValue.RData")

# Adding two GO analysis methods as reviewers' request (WebGestalt and GOstats)
